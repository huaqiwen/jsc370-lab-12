...
p3_scatter <- cv_states_today %>% 
  plot_ly(x = ~pop_density, y = ~newper100k,
          type = 'scatter', mode = 'markers', color = ~state,
          size = ~population, sizes = c(5, 70), marker = list(sizemode='diameter', opacity=0.5),
          hoverinfo = 'text',
          text = ~paste( paste(state, ":", sep=""), paste(" New cases per 100k: ", per100k, sep="") , paste(" Deaths per 100k: ",
                        deathsper100k, sep=""), sep = "<br>")) %>%
  layout(title = "Population-normalized COVID-19 new cases vs. population density",
                  yaxis = list(title = "New cases per 100k"), xaxis = list(title = "Population Density"),
         hovermode = "compare")
```
